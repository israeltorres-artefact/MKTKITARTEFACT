...
def scan_assets():
    ...
    for f in sorted(glob.glob(os.path.join(BASE_DIR, "assets/logos/*.png"))):
        ...
        desc = "Logotipo oficial en versión oscura para diapositivas con fondo blanco o claro." if not is_white else "Logotipo oficial en versión blanca para diapositivas oscuras o degradados."
    ...
    glassy_meta = {
        ...
        "icon_glassy_people.png": {
            ...
            "topic": "Capacitación en Inteligencia Artificial, Cultura Data-Driven, Gestión del Cambio y Equipos.",
            "color": "#273275"
        },
...
<head>
  ...
  <title>Artefact Brand System & Executive Asset Hub</title>
  ...
  <style>
    ...
    * {{ box-sizing: border-box; margin: 0; padding: 0; }}
    ...
    .ambient-glow {{
      ...
      width: 1000px;
      height: 600px;
      background: radial-gradient(circle, rgba(255, 0, 102, 0.18) 0%, rgba(39, 50, 117, 0.15) 50%, transparent 80%);
      filter: blur(80px);
      z-index: 0;
      pointer-events: none;
    }}
...
<body>
  ...
  <header class="navbar">
    <a href="#" class="nav-brand">
      ...
      <div class="nav-brand-text">
        <span>Artefact Design System</span>
        <span>Data & AI Executive Hub</span>
      </div>
    </a>
    <div class="nav-actions">
      <button class="btn-nav btn-nav-secondary" onclick="copyMasterPrompt()">📋 Copiar Prompt LLM</button>
      <a href="docs/ELEMENTS_AND_CONTEXT_PLAYBOOK.md" class="btn-nav btn-nav-secondary" target="_blank">📖 Playbook</a>
      <a href="artefact-llm-presentation-kit-v1.zip" class="btn-nav btn-nav-primary" download>📦 Descargar Kit ZIP</a>
    </div>
  </header>
  ...
  <div class="container">
    ...
    <section class="hero">
      <div class="hero-badge">Official Release • November 2024 • Cloud CDN Active</div>
      <h1 class="hero-title">
        El Sistema de Identidad & Recursos de <strong>Artefact</strong>
      </h1>
      <p class="hero-subtitle">
        Catálogo centralizado de logotipos de alta resolución, iconos 3D Glassy, vectores SVG puros, banco de fotografía por industrias y tokens de color listos para su uso directo en tus LLMs y compilador PowerPoint.
      </p>
    ...
    <div class="sticky-toolbar">
      <div class="search-wrapper">
        ...
        <input type="text" class="search-input" id="globalSearch" placeholder="Buscar activo por nombre, industria (Retail, AI, Banca, ESG) o etiqueta..." oninput="onSearchInput()">
      </div>
    ...
    <div class="tabs-nav" id="categoryTabs">
      ...
      <button class="tab-pill" onclick="setCategory('archetypes')">📑 Arquetipos de Slides</button>
    </div>
      ...
      <div class="color-grid">
        ...
        <div class="color-card-item" onclick="copyValue('#F4F6F9', 'Surface Light')">
          ...
          <div class="color-meta">
            ...
            <div class="color-role">Superficie estándar de tarjetas modulares en diapositivas blancas.</div>
          </div>
        </div>
      </div>
    </div>
    ...
    <div id="typographySection" style="display: none; margin-bottom: 40px;">
      <div class="section-title-box">
        ...
        <div class="section-subtitle-text">Escala exacta para presentaciones en formato 16:9 (1920x1080)</div>
      </div>
      ...
      <div style="background: var(--bg-surface); border: 1px solid var(--border-dark); border-radius: 18px; padding: 36px;">
        ...
        <div style="display: flex; gap: 48px; align-items: baseline; flex-wrap: wrap;">
          ...
          <div>
            ...
            <div style="font-size: 12px; color: var(--text-muted); margin-top: 6px;"><code>Roboto Medium (500) • 10pt</code> (Pie de página / Fuente)</div>
          </div>
        </div>
      </div>
    </div>

    <div id="archetypesSection" style="display: none; margin-bottom: 40px;">
      <div class="section-title-box">
        <div class="section-title-text">📑 Arquetipos de Diapositivas de Consultoría Artefact</div>
        <div class="section-subtitle-text">Patrones de diseño listos para implementar en Presentation HTML</div>
      </div>

      <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(320px, 1fr)); gap: 24px;">
        <div class="stat-card" style="text-align: left; padding: 24px;">
          <div style="font-size: 12px; font-weight: 700; color: var(--accent-pink); text-transform: uppercase;">Arquetipo 01</div>
          <h3 style="font-size: 18px; margin: 8px 0; color: white;">Portada Hero con Degradado</h3>
          <p style="font-size: 13px; color: var(--text-muted); line-height: 1.5; margin-bottom: 16px;">
            Fondo en degradado insigne de 4 paradas, logotipo blanco, título en 54pt e icono 3D Glassy gigante (AI Hero).
          </p>
          <button class="btn-nav btn-nav-secondary" style="font-size: 12px; width: 100%; justify-content: center;" onclick="copyTemplateCode('cover')">Copiar Código HTML</button>
        </div>

        <div class="stat-card" style="text-align: left; padding: 24px;">
          <div style="font-size: 12px; font-weight: 700; color: var(--accent-pink); text-transform: uppercase;">Arquetipo 02</div>
          <h3 style="font-size: 18px; margin: 8px 0; color: white;">Pilares de Capacidades (3 Cols)</h3>
          <p style="font-size: 13px; color: var(--text-muted); line-height: 1.5; margin-bottom: 16px;">
            Fondo blanco, 3 tarjetas de superficie con iconos 3D Glassy superiores (AI, BI, Strategy) y bullets en negrita.
          </p>
          <button class="btn-nav btn-nav-secondary" style="font-size: 12px; width: 100%; justify-content: center;" onclick="copyTemplateCode('pillars')">Copiar Código HTML</button>
        </div>

        <div class="stat-card" style="text-align: left; padding: 24px;">
          <div style="font-size: 12px; font-weight: 700; color: var(--accent-pink); text-transform: uppercase;">Arquetipo 03</div>
          <h3 style="font-size: 18px; margin: 8px 0; color: white;">Caso de Éxito de Cliente</h3>
          <p style="font-size: 13px; color: var(--text-muted); line-height: 1.5; margin-bottom: 16px;">
            Layout dividido 50/50: Desafío y Solución a la izquierda, tarjeta azul marino con KPI gigante rosa a la derecha.
          </p>
          <button class="btn-nav btn-nav-secondary" style="font-size: 12px; width: 100%; justify-content: center;" onclick="copyTemplateCode('case')">Copiar Código HTML</button>
        </div>
      </div>
    </div>
    ...
    <div id="assetsGridSection">
      <div class="section-title-box">
        ...
        <div class="section-subtitle-text">Haz clic en cualquier tarjeta para abrir el inspector de código</div>
      </div>
  ...
  <div class="modal-overlay" id="modalOverlay" onclick="closeModal(event)">
    <div class="modal-box" onclick="event.stopPropagation()">
      ...
      <div class="modal-content-area">
        ...
        <div>
          <div style="font-size: 12px; font-weight: 700; margin-bottom: 8px; color: var(--accent-pink); text-transform: uppercase;">Código HTML (dom-to-pptx compatible):</div>
        ...
        <div>
          <div style="font-size: 12px; font-weight: 700; margin-bottom: 8px; color: var(--accent-pink); text-transform: uppercase;">URL Pública Directa (GitHub Pages):</div>
          <div class="code-block">
  ...
  <script>
    ...
    function renderAssets() {{
      ...
      archetypesSection.style.display = (currentCat === 'all' || currentCat === 'archetypes') ? 'block' : 'none';
      photoSub.style.display = (currentCat === 'photography') ? 'flex' : 'none';

      if (['colors', 'typography', 'archetypes'].includes(currentCat)) {{
    ...
    function openModal(name) {{
      ...
      document.getElementById('mSnippetHtml').innerText = `<img src="${{item.cdn_url}}" data-pptx-role="${{item.category === 'Logotipos' ? 'logo' : 'image'}}" alt="${{item.title}}">`;
    ...
    function copySnippet(type) {{
      ...
      if (type === 'html') {{
        copyValue(`<img src="${{selectedItem.cdn_url}}" data-pptx-role="${{selectedItem.category === 'Logotipos' ? 'logo' : 'image'}}" alt="${{selectedItem.title}}">`, 'Snippet HTML');
      }} else if (type === 'url') {{
        copyValue(selectedItem.cdn_url, 'URL de la CDN');
      }}
    }}
    ...
    function copyMasterPrompt() {{
      const promptText = `Eres el Director Visual y Consultor Estratégico Senior de Artefact (Data, AI & Digital Transformation).\\nUsa siempre los assets oficiales de Artefact en la nube:\\n- Base Assets: https://israeltorres-artefact.github.io/MKTKITARTEFACT/assets/\\n- Logo Blanco: https://israeltorres-artefact.github.io/MKTKITARTEFACT/assets/logos/artefact_logo_primary_white.png\\n- Iconos 3D: https://israeltorres-artefact.github.io/MKTKITARTEFACT/assets/icons/glassy_3d/icon_glassy_ai_acceleration.png\\n- Paleta: #002244 (Azul), #FF0066 (Rosa), #0D1634 (Dark Blue)\\n- Tipografía: Roboto Normal 20pt (Títulos), Roboto Bold 14pt (Subtítulos), Roboto Black 36-44pt (KPIs).`;
      copyValue(promptText, 'System Prompt Maestro para LLMs');
    }}

    function copyTemplateCode(type) {{
      let code = '';
      if (type === 'cover') {{
        code = `<section class="slide gradient-theme" data-pptx-slide data-slide-id="S01" style="width:1920px;height:1080px;position:relative;overflow:hidden;background:linear-gradient(135deg, #002244 0%, #273275 33%, #752E7D 66%, #FF0066 100%);color:#FFFFFF;padding:60px 80px;">\\n  <div style="position:absolute;top:60px;left:80px;"><img src="${{CDN_BASE}}logos/artefact_logo_primary_white.png" data-pptx-role="logo" style="height:48px;"></div>\\n  <div style="position:absolute;top:320px;left:80px;max-width:1050px;">\\n    <h1 data-pptx-role="title" style="font-size:54px;font-weight:300;margin-bottom:20px;">Aceleración de IA & Datos</h1>\\n    <p data-pptx-role="subtitle" style="font-size:22px;opacity:0.9;">Estrategia integral de transformación digital.</p>\\n  </div>\\n  <div style="position:absolute;right:100px;top:240px;width:440px;height:440px;"><img src="${{CDN_BASE}}icons/glassy_3d/icon_glassy_ai_hero_large.png" style="max-width:100%;"></div>\\n</section>`;
      }} else if (type === 'pillars') {{
        code = `<section class="slide" data-pptx-slide data-slide-id="S02" style="width:1920px;height:1080px;position:relative;overflow:hidden;padding:60px 80px;background:#FFFFFF;">\\n  <div class="slide-header" style="margin-bottom:36px;"><h2 data-pptx-role="title" style="font-size:28px;font-weight:400;color:#002244;">Pilares de Capacidades</h2><div data-pptx-role="subtitle" style="font-size:19px;font-weight:700;color:#002244;">Tres palancas para transformar el modelo operativo</div></div>\\n  <div style="display:grid;grid-template-columns:repeat(3,1fr);gap:28px;height:720px;">\\n    <div style="background:#F4F6F9;border:1px solid #EEEEEE;border-radius:12px;padding:32px;"><img src="${{CDN_BASE}}icons/glassy_3d/icon_glassy_data_foundations_bi.png" style="width:64px;height:64px;margin-bottom:16px;"><h3 style="font-size:22px;font-weight:700;color:#002244;">Data Foundations</h3></div>\\n    <div style="background:#FFFFFF;border:2px solid #FF0066;border-radius:12px;padding:32px;"><img src="${{CDN_BASE}}icons/glassy_3d/icon_glassy_ai_acceleration.png" style="width:64px;height:64px;margin-bottom:16px;"><h3 style="font-size:22px;font-weight:700;color:#002244;">AI Acceleration</h3></div>\\n    <div style="background:#F4F6F9;border:1px solid #EEEEEE;border-radius:12px;padding:32px;"><img src="${{CDN_BASE}}icons/glassy_3d/icon_glassy_strategy_transformation.png" style="width:64px;height:64px;margin-bottom:16px;"><h3 style="font-size:22px;font-weight:700;color:#002244;">Strategy & Change</h3></div>\\n  </div>\\n</section>`;
      }} else if (type === 'case') {{
        code = `<section class="slide" data-pptx-slide data-slide-id="S03" style="width:1920px;height:1080px;position:relative;overflow:hidden;padding:60px 80px;background:#FFFFFF;">\\n  <div class="slide-header" style="margin-bottom:36px;"><h2 data-pptx-role="title" style="font-size:28px;font-weight:400;color:#002244;">Caso de Éxito de Cliente</h2></div>\\n  <div style="display:grid;grid-template-columns:1fr 1fr;gap:32px;height:720px;">\\n    <div style="background:#F4F6F9;border:1px solid #EEEEEE;border-radius:12px;padding:32px;"><h3 style="font-size:20px;font-weight:700;color:#002244;">El Desafío & Solución</h3></div>\\n    <div style="background:#002244;color:white;border-radius:12px;padding:32px;text-align:center;display:flex;flex-direction:column;justify-content:center;"><div style="font-size:72px;font-weight:900;color:#FF0066;">+22%</div><div style="font-size:22px;font-weight:700;">Ingresos Incrementales</div></div>\\n  </div>\\n</section>`;
      }}
      copyValue(code, 'Plantilla de Slide');
    }}
...
# Mirror to company kit and desktop
shutil.copyfile(os.path.join(BASE_DIR, "index.html"), os.path.join(KIT_DIR, "index.html"))
shutil.copyfile(os.path.join(BASE_DIR, "index.html"), os.path.join(DESKTOP_DIR, "company-kits", "artefact", "index.html"))

print("Epic Hub successfully generated and mirrored!")
